...
pbs2<- function(nmax,T1, T2, hv, mat, p1=0.05,p2=0.05, delta=1.5, pop.size=1000)
  {
  if (!requireNamespace("rgenoud", quietly = TRUE)) {
    stop("Package \"rgenoud\" must be installed to use this function.",
      call. = FALSE)}
  if(missing(p1))
  {p1 <- 0.05}
  if(missing(p2))
  ...
  {pop.size <- 1000}
    ...
    n1<- ((Fval <-1)*1)*n
    h1<- ((Fval <-1)*1)*h
    output <- structure(c(L, n1, h1, Fval), names= c("l", "n","h", "FVAL"))
    class(output)<- "pbs2"
    return (output)
  }
